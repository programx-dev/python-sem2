...
class JsonlSource:
    """Читает задачи из файла формата jsonl."""

    def __init__(self, path: Path | str):
        self.path = Path(path)

    def _parse_line(self, line: str, line_no: int) -> Task:
        """Метод для парсинга одной строки json и создания объекта Task."""
        try:
    ...
    def get_tasks(self) -> Iterable[Task]:
        """Синхронное чтение задач из файла."""
    ...
    async def get_async_tasks(self) -> AsyncIterable[Task]:
        """
        Асинхронное чтение задач из файла.
        """
        if not self.path.exists():
            raise FileNotFoundError(f"Файл не найден: {self.path}")
